...
import { useGetTopReviewedAppQuery } from '@/features/analytics/analyticsApiSlice';
...
const TopReviewedApp = () => {
  ...
  const { data, isLoading, isError, isFetching, isSuccess } =
    useGetTopReviewedAppQuery(queryFilterState);

  useLayoutEffect(() => {
    const root = am5.Root.new('topReviewedApp');
    ...
    series.columns.template.set('tooltipText', '{name} ({reviewCount})');
  ...
  return (
    <div className='px-4 py-2 h-full'>
      <p className='mb-2 text-sm font-medium'>App With Highest Reviews</p>
      ...
      <div className='w-full h-full' id='topReviewedApp'></div>
    </div>
  );
};

export default TopReviewedApp;
